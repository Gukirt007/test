  ...
  <div class="relative z-30" role="dialog" aria-modal="true">
    <div class="fixed inset-0 top-0 z-30 overflow-y-auto p-4 sm:p-6 md:p-20">
      ...
      <div :class="searchBoxClasses"
        class="mx-auto lg:mr-0  max-w-xl lg:-mt-16 xl:-mt-16 md:-mt-16 transform divide-y divide-gray-100 overflow-hidden rounded-xl bg-white dark:bg-black shadow-2xl ring-1 ring-black ring-opacity-5 transition-all">
        <div class="relative">
          ...
          <input type="text" v-model="rawQuery"
            class="h-12 w-full border-0 bg-transparent pl-11 pr-4 dark:text-white text-gray-800 placeholder-gray-400  sm:text-sm"
        ...
        <ul v-if="isFilterStarted && cards.length"
          ...
          <ul class=" text-md dark:text-white text-gray-700" v-for="item in cards" :value="item" :key="item.id" as="template">
        ...
        <div v-if="isFilterStarted && cards.length == 0" class="py-14 px-6 text-center text-sm sm:px-14">
          <p class="mt-4 font-semibold dark:text-white text-gray-900">No results found</p>
...
export default {
  ...
  methods: {
    ...
    async redirectTo(id) {
      ...
      // reset higlighted card
      setTimeout( () => this.$store.dispatch("fetchCards"), 4000)
      this.closeSearchBox();
    }
  },
};
